...
def train(conf):
    ...
    paddle.init(
        ...
        trainer_count=conf.trainer_count,  # number of threads
        seed=conf.paddle_seed)

    # network config
    cost = mLSTM_crf_network.training_net(conf)

    # create parameters
    # NOTE: 杂项 values are not initilized here, therefore, we need to
    # print 杂项 initialization info in the beginning of the first batch
